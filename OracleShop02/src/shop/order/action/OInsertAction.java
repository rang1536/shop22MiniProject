...
public class OInsertAction implements Action {
	...
	public OActionForward execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int memberMileage = Integer.parseInt(request.getParameter("memberMileage"));
		...
		*/
		...
		// 상품주문후에 남은수량에서 주문한 수량 차감위해 상품 수량도 체크해둔다.
		goods = goodsDao.gSelectOne(orderGoodsNum);
		...
		if(choiceMileage.equals("적용")){
			orderPayFinal = orderSum - (orderMileage+memberMileage);
			order.setOrderPayFinal(orderPayFinal);
			order.setOrderMileage(0);
			
			//System.out.println("최종결제금액 : "+orderPayFinal);
			result = orderDao.oInsertUseMileage(order);
		...
		// 적립선택시 구매자 아이디에 마일리지 적립시켜준다
			...
			//System.out.println("최종결제금액 : "+orderPayFinal);
			result = orderDao.oInsertSaveMileage(order);
		}
		// 상품주문등록시 주문한수량을 원래 수량에서 빼준값을 상품테이블에 바꿔준다.
		if(result == 1){
		goods.setGoodsAmount(goodsAmount-orderAmount);
		result = goodsDao.gUpdate(goods);
		// System.out.println("상품수량 최종체크 성공여부 : "+result);
		}
